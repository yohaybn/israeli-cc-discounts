...
- Adding a source by hand? Follow any existing `scrapers/*_scraper.py`: a `scrape()` entry point, registration in `scrapers/extra_sources.py`, raw capture in `scripts/save_raw_scrapers.py`, offline fixtures + tests, and a section in this README. `main.py` handles last-good-data fallback.
...
```bash
.venv/bin/python scripts/save_raw_scrapers.py --scrapers just4u
```
...
```bash
.venv/bin/python scripts/save_raw_scrapers.py --scrapers wincard_plus
```
...
```bash
.venv/bin/python scripts/save_raw_scrapers.py --scrapers flycard
```
...
```bash
.venv/bin/python scripts/save_raw_scrapers.py --scrapers reshef
```
